...
import {  useRemovePostImagesMutation, useUpdatePostMutation } from "../features/post/services/postApi"
...
const UpdatePost = () => {
  ...
  const location = useLocation();
    ...
    const {register,handleSubmit,formState:{errors}} = useForm<updatePostSchemaType>({
    defaultValues:{
      caption:location.state.data?.postCaption
    } });

    const [PreviewImages,setPreviewImages] = useState< postImages[]>(location.state?.data?.postImages || []);

    const [imagesArr,setImagesArr] = useState<File[]>([])

    const [requestRemovePostImages,] = useRemovePostImagesMutation()
        const [requestUpdatePost,{isLoading:updatingPost}] = useUpdatePostMutation()
    const navigate = useNavigate()

    const handleImages = (e: React.ChangeEvent<HTMLInputElement>)=>{
      const imgInputArr = Array.from(e.target.files ?? []);
           if(PreviewImages.length + imagesArr.length + imgInputArr.length > 3){
            return toast.warning("you can't add more than 3 images")
           }
        ...
        try {
          ...
          for(let file of imagesArr){
            formData.append('updatePostImage',file);
              ...
              const res = await requestUpdatePost(formData).unwrap()
              console.log(res)
...
<form onSubmit={handleSubmit(onSubmit)}>
  ...
  <div className="space-y-2">
    ...
    {...register('caption')}
      ...
      className={`w-full resize-none rounded-2xl border border-white/50 bg-white/60 p-4
         ...
         {errors.caption && <p className="text-xs ring-offset-red-500">{errors.caption.message}</p>}
  </div>
  ...
  <div className="mt-8 space-y-2">
    ...
    <div className="grid grid-cols-3 gap-4">
     {
      PreviewImages.length > 0 && PreviewImages.map((val)=>{
        return  <div 
         key={val.publicId}
        ...
        <div 
        onClick={()=>{removePostImage(val.publicId)}}
